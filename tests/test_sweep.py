...
@pytest.fixture(scope="module")
def small_env():
    config = tabular.garnet.Config(state_size=5, action_size=3, max_eps_len=10)
    return tabular.garnet.make(config)
...
def test_sweep_sample_state_action_pairing(small_env):
    ...
    for i in range(A * S):
        expected_state = i % S
        expected_action = i // S
        assert transition.obs[i] == expected_state, (
            f"Position {i}: expected state {expected_state}, got {transition.obs[i]}"
        )
        assert transition.act[i] == expected_action, (
            f"Position {i}: expected action {expected_action}, got {transition.act[i]}"
        )
...
def test_sweep_batch_shape_various_sizes():
    ...
    for state_size, action_size in [(3, 2), (5, 4), (10, 3)]:
        config = tabular.garnet.Config(
            state_size=state_size, action_size=action_size, max_eps_len=10
        )
        env = tabular.garnet.make(config)
        mc_sampler = mc.Mc(max_eps_len=10, env=env)
        sweeper = sweep.Sweep(mc=mc_sampler)
        ...
        assert transition.obs.shape == (expected_batch_size,), (
            f"Failed for S={state_size}, A={action_size}"
        )
...
def test_sweep_transitions_consistent_with_mdp(small_env):
    ...
    for i in range(A * S):
        ...
        assert prob > 0, (
            f"Position {i}: transition from s={s}, a={a} to s'={s_next} has prob 0"
        )
...
def test_sweep_single_state_env():
    ...
    key = jax.random.PRNGKey(0)
    config = tabular.garnet.Config(state_size=1, action_size=3, max_eps_len=10)
    env = tabular.garnet.make(config)
    mc_sampler = mc.Mc(max_eps_len=10, env=env)
    sweeper = sweep.Sweep(mc=mc_sampler)
...
def test_sweep_single_action_env():
    ...
    key = jax.random.PRNGKey(0)
    config = tabular.garnet.Config(state_size=5, action_size=1, max_eps_len=10)
    env = tabular.garnet.make(config)
    mc_sampler = mc.Mc(max_eps_len=10, env=env)
    sweeper = sweep.Sweep(mc=mc_sampler)
...
def test_sweep_larger_env(garnet_env):
    ...
    key = jax.random.PRNGKey(0)
    mc_sampler = mc.Mc(max_eps_len=100, env=garnet_env)
    sweeper = sweep.Sweep(mc=mc_sampler)
