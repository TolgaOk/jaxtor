"""Comprehensive tests for Induced Markov Chain sampler functionality."""

import chex
import jax
import jax.numpy as jnp
import pytest
from chex import dataclass
from jaxtor.sampler import imc, mc
from jaxtor.env import tabular


# ============================================================================
# Test Infrastructure
# ============================================================================


class GoRightAgent:
    """Agent that always takes the 'right' action (action index 1)."""

    @dataclass
    class State:
        pass  # Stateless agent

    def act(self, obs, state):
        action = jax.nn.one_hot(1, 4)  # 4 actions: up, right, down, left
        return action, state


class CountingAgent:
    """Agent with a counter in state that increments each step."""

    @dataclass
    class State:
        counter: int

    def __init__(self, action_size: int = 4):
        self.action_size = action_size

    def act(self, obs, state):
        action = jax.nn.one_hot(0, self.action_size)
        new_state = state.replace(counter=state.counter + 1)
        return action, new_state


class MDPAdapter:
    """Adapter to make TabularEnv compatible with mc.Env protocol."""

    def __init__(self, mdp_env):
        self.mdp_env = mdp_env

    def init(self, key):
        """Initialize and return env_state."""
        return self.mdp_env.init(key)

    def reset(self, key, env_state):
        """Reset and return (obs, env_state)."""
        obs, env_state = self.mdp_env.reset(key, env_state)
        return jax.nn.one_hot(obs, env_state.mdp.state_size), env_state

    def step(self, key, act, env_state):
        """Step and return (Step, env_state)."""
        step_result, next_mdp_state = self.mdp_env.step(key, jnp.argmax(act), env_state)
        step_result = tabular.Step(
            nobs=jax.nn.one_hot(step_result.nobs, next_mdp_state.mdp.state_size),
            rew=step_result.rew,
            term=step_result.term,
            trun=step_result.trun,
        )
        return step_result, next_mdp_state


# ============================================================================
# Test 1: Trajectory Consistency in GridWorld
# ============================================================================


def test_trajectory_consistency_gridworld():
    """Test trajectory consistency in a hallway gridworld.

    Agent always goes right in hallway #P  @#, reaches goal in 3 steps.
    Verify transitions are consecutive and terminal handling is correct.
    """
    key = jax.random.PRNGKey(0)

    # Create hallway gridworld: P at position 0, goal @ at position 3
    # 3 steps right to reach goal
    config = tabular.gridworld.Config(
        board=["######", "#P  @#", "######"],
        p_slip=0.0,
        max_episode_len=10,
    )
    mdp_env = tabular.gridworld.make(config)
    adapted_env = MDPAdapter(mdp_env)

    # Create MC and IMC samplers
    mc_sampler = mc.MarkovChain(max_episode_len=10, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=5)

    # Initialize
    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # Collect trajectory
    transitions, next_state = imc_sampler.sample(state)

    # Check shapes (5 steps)
    assert transitions.obs.shape[0] == 5
    assert transitions.act.shape[0] == 5
    assert transitions.rew.shape[0] == 5
    assert transitions.term.shape[0] == 5
    assert transitions.trun.shape[0] == 5
    assert transitions.nobs.shape[0] == 5

    # Check consecutive transitions: nobs[i] should match obs[i+1] when not terminal
    for i in range(4):
        done = jnp.logical_or(transitions.term[i], transitions.trun[i])
        if not done:
            # nobs should match the observation at next step
            assert jnp.allclose(transitions.nobs[i], transitions.obs[i + 1])

    # Episode should terminate when reaching goal (within 3-4 steps)
    # At least one step should be terminal
    has_terminal = jnp.any(transitions.term)
    # Note: in gridworld, reaching @ gives terminal=True


def test_terminal_reset_observation():
    """Verify that after terminal, next step starts from reset state.

    When a terminal occurs, the MC sampler resets the environment. The observation
    at the next step (obs[term_idx+1]) comes from the reset state, which differs
    from the terminal observation (nobs[term_idx]).
    """
    key = jax.random.PRNGKey(42)

    # Simple 1-step hallway to goal
    config = tabular.gridworld.Config(
        board=["####", "#P@#", "####"],
        p_slip=0.0,
        max_episode_len=10,
    )
    mdp_env = tabular.gridworld.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=10, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=5)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    transitions, next_state = imc_sampler.sample(state)

    # Find terminal step
    term_indices = jnp.where(transitions.term)[0]
    if len(term_indices) > 0:
        term_idx = int(term_indices[0])
        # The terminal nobs is the goal state observation
        # The next obs (if exists) should be the reset/initial state observation
        # These are different - terminal reaches goal, reset goes back to start
        if term_idx < 4:
            # Verify next step starts from a valid observation (not NaN)
            assert not jnp.any(jnp.isnan(transitions.obs[term_idx + 1]))
            # The observations before and after reset should differ
            # (terminal state vs initial state)
            # This is environment-specific but generally true for gridworld


# ============================================================================
# Test 2: Metrics Queue Refresh
# ============================================================================


def test_metrics_queue_refresh():
    """Test that metrics() clears the queues after computation."""
    key = jax.random.PRNGKey(0)

    # Short episode environment
    config = tabular.gridworld.Config(
        board=["####", "#P@#", "####"],
        p_slip=0.0,
        max_episode_len=10,
    )
    mdp_env = tabular.gridworld.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=10, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=5)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # Run rollout that completes episodes
    transitions, state = imc_sampler.sample(state)

    # Check that some episodes completed (queue has non-NaN values)
    num_completed_before = jnp.sum(~jnp.isnan(state.mc.eps_rew_queue))

    # Call metrics to get averages
    metrics, refreshed_state = imc_sampler.metrics(state)

    # Verify queues are now filled with NaN
    assert jnp.all(jnp.isnan(refreshed_state.mc.eps_rew_queue))
    assert jnp.all(jnp.isnan(refreshed_state.mc.eps_len_queue))


# ============================================================================
# Test 3: Incomplete Episode - No Episode Stats
# ============================================================================


def test_incomplete_episode_no_stats():
    """Test that rollout shorter than episode length has all NaN in queue."""
    key = jax.random.PRNGKey(0)

    # Long episode environment
    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=100,  # Very long episode
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=100, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    # seqlen=3, much shorter than episode_len
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=3)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # Run short rollout
    transitions, state = imc_sampler.sample(state)

    # No episode should complete - queue should remain all NaN
    assert jnp.all(jnp.isnan(state.mc.eps_rew_queue))
    assert jnp.all(jnp.isnan(state.mc.eps_len_queue))

    # metrics() should return NaN for averages
    metrics, _ = imc_sampler.metrics(state)
    assert jnp.isnan(metrics.avg_eps_rew)
    assert jnp.isnan(metrics.avg_eps_len)


# ============================================================================
# Test 4: Single Complete Episode Stats
# ============================================================================


def test_single_complete_episode_stats():
    """Test rollout that completes exactly 1 episode has correct stats."""
    key = jax.random.PRNGKey(0)

    # 1-step episode: move right to goal
    config = tabular.gridworld.Config(
        board=["####", "#P@#", "####"],
        p_slip=0.0,
        max_episode_len=10,
    )
    mdp_env = tabular.gridworld.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=10, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    # seqlen=5 to ensure we complete at least one episode
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=2)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # Run rollout
    transitions, state = imc_sampler.sample(state)

    # Check that first queue slot has episode data
    assert not jnp.isnan(state.mc.eps_rew_queue[0])
    assert not jnp.isnan(state.mc.eps_len_queue[0])

    # Episode length should be 1 (one step to goal)
    assert state.mc.eps_len_queue[0] == 1

    # metrics should return the single episode's values
    metrics, _ = imc_sampler.metrics(state)
    assert jnp.isclose(metrics.avg_eps_len, 1.0)


# ============================================================================
# Test 5: Multiple Complete Episodes Stats
# ============================================================================


def test_multiple_complete_episodes_stats():
    """Test rollout that completes 2-3 episodes has correct average stats."""
    key = jax.random.PRNGKey(0)

    # 1-step episode each time
    config = tabular.gridworld.Config(
        board=["####", "#P@#", "####"],
        p_slip=0.0,
        max_episode_len=10,
    )
    mdp_env = tabular.gridworld.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=10, queue_size=10, env=adapted_env)
    agent = GoRightAgent()
    # seqlen=10 to ensure multiple episodes complete
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=10)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # Run rollout
    transitions, state = imc_sampler.sample(state)

    # Count completed episodes (non-NaN entries)
    num_completed = jnp.sum(~jnp.isnan(state.mc.eps_rew_queue))
    assert num_completed >= 2  # At least 2 episodes in 10 steps with 1-step episodes

    # Each episode should have length 1
    valid_lens = state.mc.eps_len_queue[~jnp.isnan(state.mc.eps_len_queue)]
    assert jnp.all(valid_lens == 1)

    # metrics should return mean of episode values
    metrics, _ = imc_sampler.metrics(state)
    assert jnp.isclose(metrics.avg_eps_len, 1.0)


# ============================================================================
# Test 6: State Preservation Across Samples
# ============================================================================


def test_state_preservation_across_samples():
    """Test that multiple sample() calls preserve state continuity."""
    key = jax.random.PRNGKey(0)

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=20,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=20, queue_size=10, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=5)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # First rollout: 5 steps
    transitions1, state = imc_sampler.sample(state)
    eps_idx_after_first = state.mc.eps_idx
    last_obs_after_first = state.mc.last_obs

    # Second rollout: continues from same state
    transitions2, state = imc_sampler.sample(state)

    # Verify continuity: first obs of second rollout should match last state
    assert jnp.allclose(transitions2.obs[0], last_obs_after_first)

    # eps_idx should continue incrementing (unless episode ended)
    # If no episode ended in first rollout, eps_idx should be 5 after first, then 10 after second


def test_episode_reward_accumulation_across_samples():
    """Test that eps_rew accumulates correctly across sample calls."""
    key = jax.random.PRNGKey(0)

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=3)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # First sample
    transitions1, state = imc_sampler.sample(state)
    eps_rew_after_first = state.mc.eps_rew

    # Second sample (if no episode ended, reward should continue accumulating)
    # Note: This depends on whether episode ended in first sample


# ============================================================================
# Test 7: Agent State Updates
# ============================================================================


def test_agent_state_updates():
    """Test that stateful agent state increments during rollout."""
    key = jax.random.PRNGKey(0)

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = CountingAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=10)

    mc_state = mc_sampler.init(key)
    agent_state = CountingAgent.State(counter=0)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # Initial counter is 0
    assert state.agent.counter == 0

    # Run rollout
    transitions, state = imc_sampler.sample(state)

    # Agent state should have incremented 10 times (seqlen=10)
    assert state.agent.counter == 10

    # Run another rollout
    transitions, state = imc_sampler.sample(state)

    # Counter should now be 20
    assert state.agent.counter == 20


# ============================================================================
# Test 8: Output Shapes and Types
# ============================================================================


def test_output_shapes_and_types():
    """Verify rollout structure has correct shapes."""
    key = jax.random.PRNGKey(0)

    state_size = 8
    action_size = 4
    seqlen = 15

    config = tabular.garnet.Config(
        state_size=state_size,
        action_size=action_size,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=seqlen)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    transitions, next_state = imc_sampler.sample(state)

    # Check shapes
    assert transitions.obs.shape == (seqlen, state_size)
    assert transitions.act.shape == (seqlen, action_size)
    assert transitions.rew.shape == (seqlen,)
    assert transitions.term.shape == (seqlen,)
    assert transitions.trun.shape == (seqlen,)
    assert transitions.nobs.shape == (seqlen, state_size)

    # Check types are arrays
    assert isinstance(transitions.obs, jnp.ndarray)
    assert isinstance(transitions.act, jnp.ndarray)
    assert isinstance(transitions.rew, jnp.ndarray)
    assert isinstance(transitions.term, jnp.ndarray)
    assert isinstance(transitions.trun, jnp.ndarray)
    assert isinstance(transitions.nobs, jnp.ndarray)


# ============================================================================
# Test 9: JIT Compilation
# ============================================================================


def test_jit_compilation_sample():
    """Verify sample() can be JIT compiled."""
    key = jax.random.PRNGKey(0)

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=10)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # JIT compile sample
    jit_sample = jax.jit(imc_sampler.sample)

    # Run JIT compiled version
    transitions, next_state = jit_sample(state)

    # Verify correctness
    assert transitions.obs.shape == (10, 10)
    assert transitions.act.shape == (10, 4)


def test_jit_compilation_metrics():
    """Verify metrics() can be JIT compiled."""
    key = jax.random.PRNGKey(0)

    config = tabular.gridworld.Config(
        board=["####", "#P@#", "####"],
        p_slip=0.0,
        max_episode_len=10,
    )
    mdp_env = tabular.gridworld.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=10, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=5)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # Run sample to populate some data
    transitions, state = imc_sampler.sample(state)

    # JIT compile metrics
    jit_metrics = jax.jit(imc_sampler.metrics)

    # Run JIT compiled version
    metrics, refreshed_state = jit_metrics(state)

    # Verify it returns valid structure
    assert hasattr(metrics, "avg_eps_rew")
    assert hasattr(metrics, "avg_eps_len")
    assert jnp.all(jnp.isnan(refreshed_state.mc.eps_rew_queue))


def test_jit_multiple_calls():
    """Verify JIT compiled functions work correctly across multiple calls."""
    key = jax.random.PRNGKey(0)

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = CountingAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=5)

    mc_state = mc_sampler.init(key)
    agent_state = CountingAgent.State(counter=0)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    jit_sample = jax.jit(imc_sampler.sample)

    # Multiple JIT calls should maintain state correctly
    for i in range(5):
        transitions, state = jit_sample(state)
        expected_counter = (i + 1) * 5
        assert state.agent.counter == expected_counter


# ============================================================================
# Test 10: Vmap Over Batch
# ============================================================================


def test_vmap_over_batch_init():
    """Test vmap works for initializing batch of environments."""
    key = jax.random.PRNGKey(0)
    num_envs = 4

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)

    # Initialize batch of MC states
    keys = jax.random.split(key, num_envs)
    init_vmap = jax.vmap(mc_sampler.init)
    mc_states = init_vmap(keys)

    # Check batch dimensions
    assert mc_states.last_obs.shape == (num_envs, 10)
    assert mc_states.eps_rew_queue.shape == (num_envs, 5)


def test_vmap_over_batch_sample():
    """Test vmap works for sampling from batch of environments."""
    key = jax.random.PRNGKey(0)
    num_envs = 4

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=10)

    # Initialize batch of states
    keys = jax.random.split(key, num_envs)
    init_vmap = jax.vmap(mc_sampler.init)
    mc_states = init_vmap(keys)

    # Create batch of agent states (stateless, so just replicate)
    agent_states = jax.tree.map(
        lambda x: jnp.stack([x] * num_envs), GoRightAgent.State()
    )

    # Initialize IMC states
    init_imc_vmap = jax.vmap(imc_sampler.init)
    imc_states = init_imc_vmap(mc_states, agent_states)

    # Vmap sample
    sample_vmap = jax.vmap(imc_sampler.sample)
    transitions, next_states = sample_vmap(imc_states)

    # Check output shapes include batch dimension
    assert transitions.obs.shape == (num_envs, 10, 10)  # (batch, seqlen, obs_size)
    assert transitions.act.shape == (num_envs, 10, 4)  # (batch, seqlen, act_size)
    assert transitions.rew.shape == (num_envs, 10)  # (batch, seqlen)
    assert transitions.term.shape == (num_envs, 10)
    assert transitions.trun.shape == (num_envs, 10)
    assert transitions.nobs.shape == (num_envs, 10, 10)


def test_vmap_with_stateful_agent():
    """Test vmap works with stateful agent over batch."""
    key = jax.random.PRNGKey(0)
    num_envs = 4

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = CountingAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=10)

    # Initialize batch of MC states
    keys = jax.random.split(key, num_envs)
    init_vmap = jax.vmap(mc_sampler.init)
    mc_states = init_vmap(keys)

    # Create batch of agent states with different initial counters
    agent_states = CountingAgent.State(counter=jnp.arange(num_envs))

    # Initialize IMC states
    init_imc_vmap = jax.vmap(imc_sampler.init)
    imc_states = init_imc_vmap(mc_states, agent_states)

    # Check initial counters
    assert jnp.array_equal(imc_states.agent.counter, jnp.arange(num_envs))

    # Vmap sample
    sample_vmap = jax.vmap(imc_sampler.sample)
    transitions, next_states = sample_vmap(imc_states)

    # Each agent should have incremented by 10 (seqlen)
    expected_counters = jnp.arange(num_envs) + 10
    assert jnp.array_equal(next_states.agent.counter, expected_counters)


def test_vmap_metrics():
    """Test vmap works for metrics over batch."""
    key = jax.random.PRNGKey(0)
    num_envs = 4

    config = tabular.gridworld.Config(
        board=["####", "#P@#", "####"],
        p_slip=0.0,
        max_episode_len=10,
    )
    mdp_env = tabular.gridworld.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=10, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=5)

    # Initialize batch
    keys = jax.random.split(key, num_envs)
    init_vmap = jax.vmap(mc_sampler.init)
    mc_states = init_vmap(keys)

    agent_states = jax.tree.map(
        lambda x: jnp.stack([x] * num_envs), GoRightAgent.State()
    )

    init_imc_vmap = jax.vmap(imc_sampler.init)
    imc_states = init_imc_vmap(mc_states, agent_states)

    # Sample to populate data
    sample_vmap = jax.vmap(imc_sampler.sample)
    transitions, imc_states = sample_vmap(imc_states)

    # Vmap metrics
    metrics_vmap = jax.vmap(imc_sampler.metrics)
    metrics, refreshed_states = metrics_vmap(imc_states)

    # Check batch dimensions
    assert metrics.avg_eps_rew.shape == (num_envs,)
    assert metrics.avg_eps_len.shape == (num_envs,)
    assert refreshed_states.mc.eps_rew_queue.shape == (num_envs, 5)

    # All queues should be refreshed to NaN
    assert jnp.all(jnp.isnan(refreshed_states.mc.eps_rew_queue))


# ============================================================================
# Additional Edge Cases
# ============================================================================


def test_init_returns_correct_structure():
    """Test that init returns correct IMC State structure."""
    key = jax.random.PRNGKey(0)

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = CountingAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=10)

    mc_state = mc_sampler.init(key)
    agent_state = CountingAgent.State(counter=42)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    # Check structure
    assert hasattr(state, "mc")
    assert hasattr(state, "agent")
    assert state.agent.counter == 42
    assert state.mc.last_obs.shape == (10,)


def test_seqlen_one():
    """Test with seqlen=1 (single step rollout)."""
    key = jax.random.PRNGKey(0)

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = GoRightAgent()
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=1)

    mc_state = mc_sampler.init(key)
    agent_state = GoRightAgent.State()
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    transitions, next_state = imc_sampler.sample(state)

    assert transitions.obs.shape == (1, 10)
    assert transitions.rew.shape == (1,)


def test_unroll_parameter():
    """Test that _unroll parameter works correctly."""
    key = jax.random.PRNGKey(0)

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = GoRightAgent()

    # Test with different unroll values
    for unroll in [1, 2, 5]:
        imc_sampler = imc.InducedMarkovChain(
            agent=agent, mc=mc_sampler, seqlen=10, _unroll=unroll
        )

        mc_state = mc_sampler.init(key)
        agent_state = GoRightAgent.State()
        state = imc_sampler.init(mc=mc_state, agent=agent_state)

        transitions, next_state = imc_sampler.sample(state)

        # Should produce same shape regardless of unroll
        assert transitions.obs.shape == (10, 10)


@dataclass
class RandomAgent:
    """Agent that samples actions uniformly at random."""

    action_size: int

    @dataclass
    class State:
        key: chex.PRNGKey

    def act(self, obs, state):
        key, action_key = jax.random.split(state.key)
        action = jax.nn.one_hot(
            jax.random.randint(action_key, (), 0, self.action_size),
            self.action_size,
        )
        return action, state.replace(key=key)


def chi_square_uniform_test(counts, expected_count, alpha=0.01):
    """Test if counts follow uniform distribution."""
    from scipy import stats

    expected = jnp.full_like(counts, expected_count, dtype=float)
    _, p_value = stats.chisquare(counts, expected)
    return p_value > alpha


@pytest.mark.statistical
def test_state_visitation_coverage():
    """Verify RandomAgent visits at least 80% of states over many rollouts."""
    key = jax.random.PRNGKey(0)
    state_size = 10
    action_size = 4

    config = tabular.garnet.Config(
        state_size=state_size,
        action_size=action_size,
        branch_size=3,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=10, env=adapted_env)
    agent = RandomAgent(action_size=action_size)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=100)

    visited_states = jnp.zeros(state_size)
    mc_state = mc_sampler.init(key)

    for _ in range(10):
        key, agent_key = jax.random.split(key)
        agent_state = RandomAgent.State(key=agent_key)
        state = imc_sampler.init(mc=mc_state, agent=agent_state)

        transitions, state = imc_sampler.sample(state)
        mc_state = state.mc

        state_indices = jnp.argmax(transitions.obs, axis=-1)
        for idx in state_indices:
            visited_states = visited_states.at[idx].set(1)

    coverage = float(jnp.sum(visited_states) / state_size)
    assert coverage >= 0.8


@pytest.mark.statistical
def test_initial_state_distribution_uniformity():
    """Verify Garnet MDP samples initial states uniformly via chi-square test."""
    key = jax.random.PRNGKey(42)
    state_size = 10
    num_samples = 1000

    config = tabular.garnet.Config(
        state_size=state_size,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)

    initial_state_counts = jnp.zeros(state_size)
    mc_state = mc_sampler.init(key)

    for _ in range(num_samples):
        key, reset_key = jax.random.split(key)
        obs, _ = adapted_env.reset(reset_key, mc_state.env)
        state_idx = jnp.argmax(obs)
        initial_state_counts = initial_state_counts.at[state_idx].add(1)

    expected_count = num_samples / state_size
    assert chi_square_uniform_test(initial_state_counts, expected_count, alpha=0.01)

    std = float(jnp.sqrt(expected_count * (1 - 1 / state_size)))
    assert bool(jnp.all(jnp.abs(initial_state_counts - expected_count) < 3 * std))


@pytest.mark.statistical
def test_empirical_transition_frequency():
    """Verify sampled transitions match MDP transition matrix with KL divergence < 0.1."""
    key = jax.random.PRNGKey(123)
    state_size = 5
    action_size = 3
    num_samples = 500

    config = tabular.garnet.Config(
        state_size=state_size,
        action_size=action_size,
        branch_size=2,
        max_episode_len=1000,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=1000, queue_size=5, env=adapted_env)

    mc_state = mc_sampler.init(key)
    mdp = mc_state.env.mdp

    test_state = 0
    test_action = 0
    true_probs = mdp.transition[test_action, :, test_state]

    next_state_counts = jnp.zeros(state_size)

    for _ in range(num_samples):
        key, _, _ = jax.random.split(key, 3)

        forced_state = jax.nn.one_hot(test_state, state_size)
        mc_state = mc_state.replace(
            env=mc_state.env.replace(last_state=forced_state),
            last_obs=forced_state,
        )

        action = jax.nn.one_hot(test_action, action_size)
        transition, mc_state = mc_sampler.sample(action, mc_state)

        next_state_idx = jnp.argmax(transition.nobs)
        next_state_counts = next_state_counts.at[next_state_idx].add(1)

    empirical_probs = next_state_counts / num_samples

    eps = 1e-10
    kl_div = float(
        jnp.sum(true_probs * jnp.log((true_probs + eps) / (empirical_probs + eps)))
    )

    assert kl_div < 0.1


@pytest.mark.statistical
def test_reward_distribution_statistics():
    """Verify rewards are within [0, 1] bounds and mean is approximately 0.5."""
    key = jax.random.PRNGKey(0)
    min_reward = 0.0
    max_reward = 1.0
    num_samples = 1000

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        min_reward=min_reward,
        max_reward=max_reward,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=10, env=adapted_env)
    agent = RandomAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=num_samples)

    key, agent_key = jax.random.split(key)
    mc_state = mc_sampler.init(key)
    agent_state = RandomAgent.State(key=agent_key)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    transitions, _ = imc_sampler.sample(state)
    rewards = transitions.rew

    assert bool(jnp.all(rewards >= min_reward))
    assert bool(jnp.all(rewards <= max_reward))

    expected_mean = (min_reward + max_reward) / 2
    actual_mean = float(jnp.mean(rewards))
    assert abs(actual_mean - expected_mean) < 0.1


@pytest.mark.statistical
def test_episode_length_distribution_with_truncation():
    """Verify episode lengths are bounded by max_episode_len and some truncate."""
    key = jax.random.PRNGKey(0)
    max_episode_len = 20
    num_rollouts = 100

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=max_episode_len,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(
        max_episode_len=max_episode_len, queue_size=100, env=adapted_env
    )
    agent = RandomAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(
        agent=agent, mc=mc_sampler, seqlen=max_episode_len * num_rollouts
    )

    key, agent_key = jax.random.split(key)
    mc_state = mc_sampler.init(key)
    agent_state = RandomAgent.State(key=agent_key)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    _, state = imc_sampler.sample(state)

    eps_lens = state.mc.eps_len_queue[~jnp.isnan(state.mc.eps_len_queue)]

    assert bool(jnp.all(eps_lens <= max_episode_len))
    assert bool(jnp.any(eps_lens == max_episode_len))

    mean_len = float(jnp.mean(eps_lens))
    assert 1 < mean_len <= max_episode_len


@pytest.mark.statistical
def test_deterministic_reproducibility():
    """Verify identical keys produce identical trajectories."""
    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = RandomAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=20)

    key1 = jax.random.PRNGKey(42)
    mc_state1 = mc_sampler.init(key1)
    key1, agent_key1 = jax.random.split(key1)
    agent_state1 = RandomAgent.State(key=agent_key1)
    state1 = imc_sampler.init(mc=mc_state1, agent=agent_state1)
    transitions1, _ = imc_sampler.sample(state1)

    key2 = jax.random.PRNGKey(42)
    mc_state2 = mc_sampler.init(key2)
    key2, agent_key2 = jax.random.split(key2)
    agent_state2 = RandomAgent.State(key=agent_key2)
    state2 = imc_sampler.init(mc=mc_state2, agent=agent_state2)
    transitions2, _ = imc_sampler.sample(state2)

    assert bool(jnp.allclose(transitions1.obs, transitions2.obs))
    assert bool(jnp.allclose(transitions1.act, transitions2.act))
    assert bool(jnp.allclose(transitions1.rew, transitions2.rew))
    assert bool(jnp.allclose(transitions1.nobs, transitions2.nobs))


@pytest.mark.statistical
def test_stochastic_variation():
    """Verify different keys produce different trajectories with nonzero variance."""
    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=5, env=adapted_env)
    agent = RandomAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=20)

    all_obs = []
    for seed in range(10):
        key = jax.random.PRNGKey(seed)
        mc_state = mc_sampler.init(key)
        key, agent_key = jax.random.split(key)
        agent_state = RandomAgent.State(key=agent_key)
        state = imc_sampler.init(mc=mc_state, agent=agent_state)
        transitions, _ = imc_sampler.sample(state)
        all_obs.append(transitions.obs)

    all_obs = jnp.stack(all_obs)

    obs_variance = jnp.var(all_obs, axis=0)
    assert bool(jnp.any(obs_variance > 0))

    unique_first_obs = jnp.unique(jnp.argmax(all_obs[:, 0, :], axis=-1))
    assert len(unique_first_obs) > 1


@pytest.mark.statistical
def test_queue_statistics_convergence():
    """Verify metrics.avg_eps_rew and avg_eps_len match manually computed queue means."""
    key = jax.random.PRNGKey(0)
    max_episode_len = 10
    queue_size = 20

    config = tabular.garnet.Config(
        state_size=5,
        action_size=3,
        max_episode_len=max_episode_len,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(
        max_episode_len=max_episode_len, queue_size=queue_size, env=adapted_env
    )
    agent = RandomAgent(action_size=3)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=200)

    key, agent_key = jax.random.split(key)
    mc_state = mc_sampler.init(key)
    agent_state = RandomAgent.State(key=agent_key)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    _, state = imc_sampler.sample(state)

    metrics, _ = imc_sampler.metrics(state)

    valid_rew = state.mc.eps_rew_queue[~jnp.isnan(state.mc.eps_rew_queue)]
    valid_len = state.mc.eps_len_queue[~jnp.isnan(state.mc.eps_len_queue)]

    expected_avg_rew = float(jnp.mean(valid_rew))
    expected_avg_len = float(jnp.mean(valid_len))

    assert float(metrics.avg_eps_rew) == pytest.approx(expected_avg_rew, rel=1e-5)
    assert float(metrics.avg_eps_len) == pytest.approx(expected_avg_len, rel=1e-5)


@pytest.mark.statistical
def test_action_coverage_with_random_policy():
    """Verify RandomAgent samples each action at least 50% of expected frequency."""
    key = jax.random.PRNGKey(0)
    action_size = 4
    num_steps = 1000

    config = tabular.garnet.Config(
        state_size=10,
        action_size=action_size,
        max_episode_len=1000,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=1000, queue_size=5, env=adapted_env)
    agent = RandomAgent(action_size=action_size)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=num_steps)

    key, agent_key = jax.random.split(key)
    mc_state = mc_sampler.init(key)
    agent_state = RandomAgent.State(key=agent_key)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    transitions, _ = imc_sampler.sample(state)

    action_indices = jnp.argmax(transitions.act, axis=-1)
    action_counts = jnp.zeros(action_size)
    for i in range(action_size):
        action_counts = action_counts.at[i].set(jnp.sum(action_indices == i))

    expected_count = num_steps / action_size
    min_threshold = expected_count * 0.5

    assert bool(jnp.all(action_counts >= min_threshold))
    assert bool(jnp.all(action_counts < num_steps * 0.5))


@pytest.mark.statistical
def test_terminal_state_handling_consistency():
    """Verify observations after terminal are valid one-hot reset states."""
    key = jax.random.PRNGKey(0)
    max_episode_len = 5

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=max_episode_len,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(
        max_episode_len=max_episode_len, queue_size=10, env=adapted_env
    )
    agent = RandomAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=20)

    key, agent_key = jax.random.split(key)
    mc_state = mc_sampler.init(key)
    agent_state = RandomAgent.State(key=agent_key)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    transitions, _ = imc_sampler.sample(state)

    done = jnp.logical_or(transitions.term, transitions.trun)
    done_indices = jnp.where(done)[0]

    assert len(done_indices) > 0

    for done_idx in done_indices:
        if done_idx < len(transitions.obs) - 1:
            next_obs = transitions.obs[int(done_idx) + 1]
            assert float(jnp.sum(next_obs)) == pytest.approx(1.0)
            assert float(jnp.max(next_obs)) == pytest.approx(1.0)


@pytest.mark.statistical
def test_cumulative_reward_accuracy():
    """Verify first episode's summed rewards appear in the queue."""
    key = jax.random.PRNGKey(0)
    max_episode_len = 10

    config = tabular.garnet.Config(
        state_size=5,
        action_size=3,
        max_episode_len=max_episode_len,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(
        max_episode_len=max_episode_len, queue_size=10, env=adapted_env
    )
    agent = RandomAgent(action_size=3)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=50)

    key, agent_key = jax.random.split(key)
    mc_state = mc_sampler.init(key)
    agent_state = RandomAgent.State(key=agent_key)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    transitions, state = imc_sampler.sample(state)

    done = jnp.logical_or(transitions.term, transitions.trun)
    done_indices = jnp.where(done)[0]

    if len(done_indices) > 0:
        first_done = int(done_indices[0])
        episode_rewards = transitions.rew[: first_done + 1]
        computed_return = float(jnp.sum(episode_rewards))

        queue_returns = state.mc.eps_rew_queue[~jnp.isnan(state.mc.eps_rew_queue)]
        assert len(queue_returns) > 0

        assert bool(jnp.any(jnp.isclose(queue_returns, computed_return, rtol=1e-4)))


@pytest.mark.statistical
def test_batch_consistency_under_vmap():
    """Verify vmapped batch produces valid, independent trajectories with mean reward in [0.2, 0.8]."""
    key = jax.random.PRNGKey(0)
    num_envs = 16

    config = tabular.garnet.Config(
        state_size=10,
        action_size=4,
        max_episode_len=20,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=20, queue_size=5, env=adapted_env)
    agent = RandomAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=50)

    keys = jax.random.split(key, num_envs + 1)
    key, env_keys = keys[0], keys[1:]

    init_vmap = jax.vmap(mc_sampler.init)
    mc_states = init_vmap(env_keys)

    agent_keys = jax.random.split(key, num_envs)
    agent_states = RandomAgent.State(key=agent_keys)

    init_imc_vmap = jax.vmap(imc_sampler.init)
    imc_states = init_imc_vmap(mc_states, agent_states)

    sample_vmap = jax.vmap(imc_sampler.sample)
    transitions, _ = sample_vmap(imc_states)

    assert transitions.obs.shape == (num_envs, 50, 10)
    assert not bool(jnp.any(jnp.isnan(transitions.obs)))
    assert not bool(jnp.any(jnp.isnan(transitions.rew)))

    first_obs = transitions.obs[:, 0, :]
    first_obs_indices = jnp.argmax(first_obs, axis=-1)
    unique_starts = len(jnp.unique(first_obs_indices))
    assert unique_starts > 1

    batch_mean_rew = float(jnp.mean(transitions.rew))
    assert 0.2 < batch_mean_rew < 0.8


@pytest.mark.statistical
def test_long_rollout_stability():
    """Verify 1000-step rollout has no NaN values and valid state indices."""
    key = jax.random.PRNGKey(0)
    state_size = 10
    seqlen = 1000

    config = tabular.garnet.Config(
        state_size=state_size,
        action_size=4,
        max_episode_len=50,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=50, queue_size=100, env=adapted_env)
    agent = RandomAgent(action_size=4)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=seqlen)

    key, agent_key = jax.random.split(key)
    mc_state = mc_sampler.init(key)
    agent_state = RandomAgent.State(key=agent_key)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    transitions, _ = imc_sampler.sample(state)

    assert not bool(jnp.any(jnp.isnan(transitions.obs)))
    assert not bool(jnp.any(jnp.isnan(transitions.rew)))
    assert not bool(jnp.any(jnp.isnan(transitions.nobs)))

    state_indices = jnp.argmax(transitions.obs, axis=-1)
    assert bool(jnp.all(state_indices >= 0))
    assert bool(jnp.all(state_indices < state_size))

    assert transitions.obs.shape == (seqlen, state_size)
    assert transitions.rew.shape == (seqlen,)


@pytest.mark.statistical
def test_multi_episode_return_variance():
    """Verify at least 10 episodes complete with nonzero return variance."""
    key = jax.random.PRNGKey(0)
    max_episode_len = 10

    config = tabular.garnet.Config(
        state_size=5,
        action_size=3,
        min_reward=0.0,
        max_reward=1.0,
        max_episode_len=max_episode_len,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(
        max_episode_len=max_episode_len, queue_size=50, env=adapted_env
    )
    agent = RandomAgent(action_size=3)
    imc_sampler = imc.InducedMarkovChain(agent=agent, mc=mc_sampler, seqlen=500)

    key, agent_key = jax.random.split(key)
    mc_state = mc_sampler.init(key)
    agent_state = RandomAgent.State(key=agent_key)
    state = imc_sampler.init(mc=mc_state, agent=agent_state)

    _, state = imc_sampler.sample(state)

    returns = state.mc.eps_rew_queue[~jnp.isnan(state.mc.eps_rew_queue)]

    return_variance = float(jnp.var(returns))
    assert return_variance > 0

    assert len(returns) >= 10


@pytest.mark.statistical
def test_state_transition_markov_property():
    """Verify empirical transition distribution matches MDP matrix within 0.15 max diff."""
    key = jax.random.PRNGKey(0)
    state_size = 5
    action_size = 3
    num_samples = 200

    config = tabular.garnet.Config(
        state_size=state_size,
        action_size=action_size,
        max_episode_len=1000,
    )
    mdp_env = tabular.garnet.make(config)
    adapted_env = MDPAdapter(mdp_env)

    mc_sampler = mc.MarkovChain(max_episode_len=1000, queue_size=5, env=adapted_env)

    mc_state = mc_sampler.init(key)
    mdp = mc_state.env.mdp

    test_state = 2
    test_action = 1

    true_probs = mdp.transition[test_action, :, test_state]

    next_state_counts = jnp.zeros(state_size)

    for i in range(num_samples):
        key, _ = jax.random.split(key)

        forced_state = jax.nn.one_hot(test_state, state_size)
        mc_state = mc_state.replace(
            env=mc_state.env.replace(last_state=forced_state),
            last_obs=forced_state,
            eps_idx=i % 10,
        )

        action = jax.nn.one_hot(test_action, action_size)
        transition, mc_state = mc_sampler.sample(action, mc_state)

        next_state_idx = jnp.argmax(transition.nobs)
        next_state_counts = next_state_counts.at[next_state_idx].add(1)

    empirical_probs = next_state_counts / num_samples

    max_diff = float(jnp.max(jnp.abs(empirical_probs - true_probs)))
    assert max_diff < 0.15
